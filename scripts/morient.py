...
def fav():
    ...
    ##    ms = mdl.sequences()
    ##    point_chain = ms[0]
    ##    barb_chain = ms[len(ms)-1]

##    np = mdl.atoms[0].
##    la = [mdl.atoms[len(mdl.atoms)-1]]
##    cfa = numpyArrayFromAtoms(fa, xformed=True)
##    cla = numpyArrayFromAtoms(la, xformed=True)
##
##    avnp = cfa - cla
##    av = avnp[0]
##    fav = Vector(av[0],av[1],av[2])
##    return fav

##    mos = mdl.openState
##
##    y_axis = Vector(0,1,0)
##    delta = angle(y_axis, fav)
##    rotAxis = cross(y_axis, fav)
##    rot = Xform.rotation(rotAxis, -delta)
##    mos.globalXform(rot)


#####using sequence similarity to identify the first actin chain###
##m = chimera.selection.currentMolecules()[0]
##chain_objs = m.sequences()
##for cobj in chain_objs:
##    cseq = str(cobj)
##    for i in chain_key:
...
##
##
##print fAco.name #chain ID including string "chain " in front
##
##fAcid = fAco.name[len(fAco.name)-1] #single letter of chain ID
##
###finding which atoms belong to first actin chain (fAcid)
##ma = m.atoms #list of all atoms in pdb
##fAcYs = {}
##for a in ma:
##    if a.residue.id.chainId == fAcid:
##        atom = [a]
##        na = numpyArrayFromAtoms(atom, xformed=True)
##        ay = na[0][1]
##        fAcYs[ay]=a
##
##np = fAcYs[max(fAcYs)]  #northmost atom obj in first actin chain
##np_crd = np.xformCoord()
##
##angdict={}
##for a in ma:
##    if a.residue.id.chainId == fAcid and str(a) != str(np):
##        a_crd = a.xformCoord()
##        vec = np_crd - a_crd
##        y_axis = Vector(0,1,0)
##        delta = angle(vec, y_axis)
##        if delta < 7:
##            angdict[delta]=a
##            
##qd={}
##for delta in angdict:
##    a_crd = angdict[delta].xformCoord()
##    ay = a_crd.y
##    if ay < 20:
##        q = delta * ay
##        a = angdict[delta]
##        print str(angdict[delta]), delta, ay, round(q,2)
##        qd[q]=a
##        


        
##sp = fAcYs[min(fAcYs)]  #southmost atom obj in first actin chain
##sp_crd = sp.xformCoord()
##
##axv = np_crd - sp_crd
##
##
##
##print angle(axv,y_axis)
